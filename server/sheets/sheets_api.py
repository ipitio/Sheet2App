...
def create_spreadsheet(title):
    try:
        ...
        spreadsheet = service.spreadsheets().create(body=request_body).execute()
...
def get_data(spreadsheet_id, range=None, majorDimension="ROWS"):
    try:
        ...
        # If no range is specified, default to the first sheet in the spreadsheet.
        if range is None:
            range = (
        ...
        # Execute the API call
        result = service.spreadsheets().values().get(**request_body).execute()
...
def update_cell(spreadsheet_id, value_to_update, row_index, column_index):
    try:
        ...
        request_body = {
            ...
            # with the specified value, so we only need one request.
            "requests": [
                {
                    "updateCells": {
                        "range": {
            ...
            "requests": [
                {
                    "updateCells": {
                        ...
                        # Overwrite all fields, regardless of their current data value
                        "fields": "*",
                        "start": {"rowIndex": row_index, "columnIndex": 0},
                    }
                }
            ]
        }
